...
import {
  ...
  fcmNotificationsFailed,
  fcmNotificationDuration,
...
export const sendToDevice = async ({
  ...
  // Phase 7: track FCM call latency
  const _fcmStart = process.hrtime.bigint();
  ...
  try {
    ...
    const _fcmDur = Number(process.hrtime.bigint() - _fcmStart) / 1e9;
    fcmNotificationDuration.observe(_fcmDur);
    ...
    if (isTransientFcmError(error)) {
      fcmNotificationsFailed.inc({ reason: 'transient' });
    ...
    fcmNotificationsFailed.inc({ reason: 'unknown' });
