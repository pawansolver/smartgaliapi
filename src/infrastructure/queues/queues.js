...
// â”€â”€ Feed Analytics Queue (Phase 10) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let feedAnalyticsQueue = null;

/**
 * Lazily create the feed-analytics Queue (producer side).
 * Used for async batch-views processing.
 * Returns null if Redis is unavailable (graceful degradation).
 */
export const getFeedAnalyticsQueue = () => {
  try {
    if (!feedAnalyticsQueue) {
      feedAnalyticsQueue = new Queue('feed-analytics', {
        connection: getSharedBullConnection(),
        prefix: queueConfig.prefix,
        defaultJobOptions: {
          attempts: 3,
      ...
      feedAnalyticsQueue.on('error', (err) => {
        logger.error('QUEUE', 'feed_analytics_queue_error', { error: err.message });
      });
    }
    return feedAnalyticsQueue;
  } catch {
    return null; // Redis unavailable - batch views will be skipped gracefully
  }
};

export const closeFeedAnalyticsQueue = async () => {
  if (!feedAnalyticsQueue) return;
  const q = feedAnalyticsQueue;
  feedAnalyticsQueue = null;
  await q.close();
};
