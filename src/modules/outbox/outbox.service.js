 ...
 * Idempotent on (event_type, aggregate_type, aggregate_id) unique constraint.
 */
export const createEvent = async (eventData, options = {}) => {
  ...
  try {
    ...
    if (
      ...
      // Idempotent: event for this aggregate already registered
      const existing = await OutboxEvent.findOne({
        where: { event_type, aggregate_type, aggregate_id },
